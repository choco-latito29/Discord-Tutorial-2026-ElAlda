...
const progress = isLive ? 1 : Math.min(position / duration, 1);

const progressBarWidth = contentWidth - 20;
...
const totalTime = isLive ? 'LIVE' : this.formatDuration(duration);
                ...
                ctx.textAlign = 'left';
                ctx.textBaseline = 'top';
                ctx.fillText(currentTime, progressBarX, currentY);
                ctx.textAlign = 'right';
                ctx.fillText(totalTime, progressBarX + progressBarWidth, currentY);
                ctx.restore();
                ...
                ctx.fillText('Elaldas Music', width - margin - 40, bottomY);
